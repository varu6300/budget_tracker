...
export default function IncomePage(){
  ...
  async function load(){
    setLoading(true);
    try{
      const data = await getTransactionHistory({ page:0, size:50, type:'INCOME' });
        ...
        <div style={{background:'#fff', borderRadius:'16px', padding:'28px', marginBottom:'32px', boxShadow:'0 2px 12px rgba(0,0,0,0.08)'}}>
          ...
          <form onSubmit={submit} style={{display:'grid', gridTemplateColumns:'1fr 2fr 1fr auto', gap:'16px', alignItems:'end'}}>
            ...
            <div>
              ...
              <input placeholder="e.g., Salary, Freelance" value={form.description} onChange={e=>setForm(f=>({...f, description:e.target.value}))} style={{width:'100%', padding:'12px', borderRadius:'8px', border:'1.5px solid #e5e7eb', fontSize:'1rem', background:'#f9fafb'}} />
            </div>
            <div>
            ...
            <button type="submit" style={{padding:'12px 32px', background:'linear-gradient(135deg, #10b981 0%, #059669 100%)', color:'#fff', border:'none', borderRadius:'8px', fontWeight:600, fontSize:'1rem', cursor:'pointer', boxShadow:'0 2px 8px rgba(16, 185, 129, 0.3)'}}>Add Income</button>
          </form>
        </div>
        <div style={{background:'#fff', borderRadius:'16px', padding:'28px', boxShadow:'0 2px 12px rgba(0,0,0,0.08)'}}>
          <h3 style={{fontSize:'1.2rem', fontWeight:700, marginBottom:'20px', color:'#1f2937'}}>📋 Income History</h3>
          {loading ? <div style={{textAlign:'center', padding:'40px', color:'#9ca3af'}}>Loading...</div> : items.length === 0 ? <div style={{textAlign:'center', padding:'40px', color:'#9ca3af'}}>No income records yet. Add your first income above!</div> : (
